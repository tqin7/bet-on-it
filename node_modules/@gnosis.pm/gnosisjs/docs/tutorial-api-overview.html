...
<head>
    ...
    <title>API Overview - Documentation</title>
...
<div id="main">
    ...
    <h1 class="page-title">API Overview</h1>
...
<article>
    <p>The Gnosis.js library is encapsulated inside of the <a href="Gnosis.html">Gnosis</a> class. In order for it to function, it must be connected to an Ethereum network through a <a href="https://github.com/ethereum/wiki/wiki/JavaScript-API">Web3.js</a> interface. It also uses <a href="https://ipfs.io/">IPFS</a> for publishing and retrieving event data, and so it will also have to be connected to an IPFS node. Configuration of these connections can be done with a call to the asynchronous factory function <a href="Gnosis.html#.create">Gnosis.create</a>. For example, the following code will store an instance of the Gnosis.js library into the variable <code>gnosis</code>:</p>
<pre class="prettyprint source lang-js"><code>let gnosis
...
// note that gnosis is NOT guaranteed to be initialized outside the callback scope here</code></pre><p>Because of the library's dependence on remote service providers and the necessity to wait for transactions to complete on the blockchain, the majority of the methods in the API are asynchronous and return thenables in the form of <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Using_promises">Promises</a>.</p>
<p>Gnosis.js also relies on <a href="https://github.com/trufflesuite/truffle-contract">Truffle contract abstractions</a>. In fact, much of the underlying core contract functionality can be accessed in Gnosis.js as one of these abstractions. Since the Truffle contract wrapper has to perform asynchronous actions such as wait on the result of a remote request to an Ethereum RPC node, it also uses thenables. For example, here is how to use the on-chain Gnosis <a href="https://gnosis.github.io/gnosis-contracts/docs/Math/">Math</a> library exposed at <a href="Gnosis.html#contracts">Gnosis#contracts</a> to print the approximate natural log of a number:</p>
<pre class="prettyprint source lang-js"><code>const ONE = Math.pow(2, 64)
Gnosis.create()
    .then(gnosis => gnosis.contracts.Math.deployed())
    .then(math => math.ln(3 * ONE))
    .then(result => console.log('Math.ln(3) =', result.valueOf() / ONE))</code></pre><p>Although it is not strictly necessary, usage of <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/async_function"><code>async/await</code></a> syntax is encouraged for simplifying the use of thenable programming, especially in complex flow scenarios. To increase the readability of code examples from this point forward, this guide will assume <code>async/await</code> is available and snippets execute in the context of an <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/async_function"><code>async function</code></a>. With those assumptions, the previous example can be expressed like so:</p>
<pre class="prettyprint source lang-js"><code>const ONE = Math.pow(2, 64)
const gnosis = await Gnosis.create()
const math = await gnosis.contracts.Math.deployed()
console.log('Math.ln(3) =', (await math.ln(3 * ONE)).valueOf() / ONE)</code></pre><p>Gnosis.js also exposes a number of convenience methods wrapping contract operations such as <a href="Gnosis.html#createCentralizedOracle">Gnosis#createCentralizedOracle</a> and <a href="Gnosis.html#createScalarEvent">Gnosis#createScalarEvent</a>.</p>
</article>
